$(function () {
    ...
    if ($('#tickets1').length) {
        ...
        var chart = new ApexCharts(document.querySelector("#tickets1"), options);
        chart.render();
    }
    ...
    if ($('#tickets2').length) {
        ...
        var chart = new ApexCharts(document.querySelector("#tickets2"), options);
        chart.render();
    }
    ...
    if ($('#rentarreas').length) {
        ...
        var chart = new ApexCharts(document.querySelector("#rentarreas"), options);
        chart.render();
    }

    if ($('#houseoccupancy').length) {
        var options = {
            chart: {
                // height: 300,
                type: "radialBar",
                offsetY: -10, // Adjust the offsetY to center the gauge vertically
                toolbar: {
                    show: false
                }
            },
            ...
            colors: [colors.success], // Change the color if needed
            series: [96], // Adjust the value of the gauge here
            labels: ["Used"],
            responsive: [{
                breakpoint: 480,
                options: {
        ...
        var chart = new ApexCharts(document.querySelector("#houseoccupancy"), options);
        chart.render();
    }


    if ($('#ticketdoghnut').length) {
        // Sample data for distribution
        var data = [30, 20, 50];

        var distributionChart = new Chart($('#ticketdoghnut'), {
            type: 'doughnut',
            data: {
                datasets: [{
                    data: data,
                    backgroundColor: ["#ffcc00", "#0099ff", "#ff5050"] // Colors for each segment
                }],
                labels: ['Category 1', 'Category 2', 'Category 3'] // Labels for each segment
            },
            options: {
                ...
                tooltips: {
                    enabled: true, // Toggle tooltips
                    mode: 'single', // Display one tooltip per dataset
                    callbacks: {
    ...
    if ($('#ticketdonut').length) {
        ...
        var chart = new ApexCharts(document.querySelector("#ticketdonut"), options);
        chart.render();
    }
    ...
    if ($('#ticketNos').length) {
        function fetchTicketData() {
            return $.ajax({
                type: 'GET',
                url: '../../estates-dashboard-2/api/total-count-tickets.php?selected_date=',
                dataType: 'json'
            });
        }
        fetchTicketData().then(response => {
            var series = [response.total_closed_tickets, response.total_pending_tickets, response.total_open_tickets];
            console.log(series);
            var labels = ['Closed', 'Pending', 'Open']
            var colors = ['#48A030', '#f6862e', '#f83c51']

            var options = {
                series: [40,30,30], // Data points for the chart
                chart: {
                    type: 'donut', // Set the chart type to donut
                    // height: 340 // Set the height of the chart
                },
                labels: labels,
                colors: colors,
                plotOptions: {
                ...
                responsive: [{
                    ...
                    options: {
                        chart: {
                            height: 240 // Set the height of the chart for smaller screens
                        },
                        plotOptions: {
                            pie: {
                                donut: {
                ...
                },
                {
                    ...
                    options: {
                        chart: {
                            height: 280 // Set the height of the chart for medium-sized screens
                        },
                        plotOptions: {
                            pie: {
                                donut: {
                    ...
                    options: {
                        chart: {
                            height: 340 // Set the height of the chart for larger screens
                        },
                        plotOptions: {
                            pie: {
                                donut: {
                    ...
                    options: {
                        chart: {
                            height: 400 // Set the height of the chart for larger screens
                        },
                        plotOptions: {
                            pie: {
                                donut: {
            ...
            // Create a new instance of ApexCharts with the provided options
            var chart = new ApexCharts(document.querySelector("#ticketNos"), options);
    ...
    if ($('#tenantdist').length) {
        var options = {
            series: [480, 180, 240, 180, 120], // Data points for the chart
            chart: {
                type: 'donut', // Set the chart type to donut
                height: 300 // Set the height of the chart
            },
            labels: ["Nairobi", "Northern", "Eastern", "Western", "Southern"],
            colors: ["#CB1D1C", "#E31ED3", "#00A30C", "#6C63FF", "#20D4DB"],
            plotOptions: {
                pie: {
                    donut: {
                        size: '60%', // Adjust the size of the hollow here
                        labels: {
                            show: true,
                            name: {
                                show: true,
        ...
        // Create a new instance of ApexCharts with the provided options
        var chart = new ApexCharts(document.querySelector("#tenantdist"), options);
